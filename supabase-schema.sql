-- 🆓 SUPABASE FREE TIER OPTIMIZED SCHEMA
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_online ON users(is_online) WHERE is_online = true;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_sender_time ON messages(sender_id, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_receiver_time ON messages(receiver_id, created_at DESC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_friends_user ON friends(user_id);
