...
const SEVERITY_KEYWORDS = {
  ...
  MEDIUM: ["spam", "fake", "inappropriate", "rude", "offensive", "bully"],
  LOW: ["annoying", "weird", "suspicious", "uncomfortable"],
};
/**
...
const calculateSeverity = (text, category, evidence) => {
  ...
  if (
    [REPORT_CATEGORIES.VIOLENCE, REPORT_CATEGORIES.UNDERAGE].includes(category)
  ) {
    severityScore += 2;
  }
...
const suggestAction = (category, severity, evidence) => {
  ...
  if (severity.level >= 4) {
    if (
      category === REPORT_CATEGORIES.VIOLENCE ||
      category === REPORT_CATEGORIES.HATE_SPEECH
    ) {
      return {
...
export const getReportStatistics = (analyzedReports) => {
  ...
  analyzedReports.forEach((report) => {
    ...
    // Count by severity
    stats.bySeverity[analysis.severity.label] =
      (stats.bySeverity[analysis.severity.label] || 0) + 1;
    // Count special cases
