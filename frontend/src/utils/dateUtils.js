...
export const getDateLabel = (date) => {
  ...
  const messageDateLocal = new Date(messageDate.toLocaleString("en-US", {timeZone: userTimezone}));
  const todayLocal = new Date(today.toLocaleString("en-US", {timeZone: userTimezone}));
  const yesterdayLocal = new Date(yesterday.toLocaleString("en-US", {timeZone: userTimezone}));

  // Reset time to compare only dates in user's timezone
  const messageDateOnly = new Date(messageDateLocal.getFullYear(), messageDateLocal.getMonth(), messageDateLocal.getDate());
  const todayOnly = new Date(todayLocal.getFullYear(), todayLocal.getMonth(), todayLocal.getDate());
  const yesterdayOnly = new Date(yesterdayLocal.getFullYear(), yesterdayLocal.getMonth(), yesterdayLocal.getDate());
...
export const isDifferentDay = (date1, date2) => {
  ...
  const d1Local = new Date(new Date(date1).toLocaleString("en-US", {timeZone: userTimezone}));
  const d2Local = new Date(new Date(date2).toLocaleString("en-US", {timeZone: userTimezone}));
  
  return d1Local.getDate() !== d2Local.getDate() || 
