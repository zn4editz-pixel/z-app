...
import { Activity, Database, Server, Cpu, AlertTriangle, CheckCircle, Zap, Brain, RefreshCw } from "lucide-react";
...
const SystemHealthPanel = () => {
    ...
    const analyzeSystem = (stats) => {
        ...
        setAiSuggestions(prev => {
            ...
            const unique = Array.from(new Map(combined.map(item => [item.message, item])).values());
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="h-32">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={data}>
                                ...
                                <Area type="monotone" dataKey="latency" stroke="#d926a9" fillOpacity={1} fill="url(#colorLatency)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                    <div className="mt-2 text-right">
                        <span className="text-2xl font-black text-primary">{data[data.length - 1]?.latency}ms</span>
                    </div>
                </div>
                ...
                <div className="bg-base-100/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-secondary/20">
                    ...
                    <div className="h-32">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={data}>
                                <Tooltip contentStyle={{ backgroundColor: '#1f2937', borderColor: '#374151' }} />
                                <Line type="step" dataKey="memory" stroke="#1fb2a6" strokeWidth={2} dot={false} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                    <div className="mt-2 text-right">
                        <span className="text-2xl font-black text-secondary">{data[data.length - 1]?.memory}%</span>
                    </div>
                </div>
                ...
                <div className="bg-base-100/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-accent/20">
                    ...
                    <div className="flex flex-col items-center justify-center h-32 space-y-2">
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center ${systemStats?.dbStatus === 'connected' || !loading ? 'bg-success/20 text-success' : 'bg-error/20 text-error'} animate-pulse`}>
                            <Server className="w-8 h-8" />
                        </div>
                        <span className="font-bold uppercase tracking-wider text-sm">{systemStats?.dbStatus || (loading ? 'checking...' : 'connected')}</span>
                        <div className="text-xs opacity-60">Latency: {systemStats?.dbLatency || 0}ms</div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-base-200 to-base-300 p-6 rounded-2xl shadow-inner border border-base-content/5">
                ...
                <div className="space-y-3">
                    {aiSuggestions.length === 0 && (
                        <div className="text-center py-8 opacity-50">AI is analyzing system metrics...</div>
                    )}
                    {aiSuggestions.map((suggestion, idx) => (
                        <div key={idx} className={`p-4 rounded-xl flex items-start gap-4 ${suggestion.type === 'critical' ? 'bg-error/10 border border-error/20' :
                            suggestion.type === 'warning' ? 'bg-warning/10 border border-warning/20' :
                                'bg-info/10 border border-info/20'
                            } animate-in slide-in-from-right duration-500`}>
                            {suggestion.type === 'critical' ? <AlertTriangle className="w-6 h-6 text-error shrink-0" /> :
                                suggestion.type === 'warning' ? <AlertTriangle className="w-6 h-6 text-warning shrink-0" /> :
                                    <CheckCircle className="w-6 h-6 text-info shrink-0" />
                            }
                            <div>
                                <h4 className="font-bold text-sm uppercase opacity-80">{suggestion.type}</h4>
