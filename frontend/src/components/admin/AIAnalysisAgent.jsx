...
const AIAnalysisAgent = () => {
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      ...
      const uniquePositive = filterUniqueInsights(
        res.data.positiveInsights || [],
        seenIssues,
      );
      const uniqueIssues = filterUniqueInsights(
        res.data.issues || [],
        seenIssues,
      );
      setAnalysis({
      ...
      if (res.data.report) {
        setReports((prev) => {
          const isDuplicate = prev.some(
            (r) => r.message === res.data.report.message,
          );
          if (!isDuplicate) {
    ...
    <div className="space-y-6">
      ...
      <div className="relative bg-gradient-to-br from-black via-yellow-900/30 to-black p-8 rounded-3xl border-2 border-yellow-500/50 shadow-2xl overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          />
          <div
            className="absolute bottom-0 left-0 w-64 h-64 bg-yellow-600/10 rounded-full blur-3xl"
            style={{
          ...
          {isAnalyzing && (
            <div
              className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-transparent via-yellow-500 to-transparent"
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div className="relative">
            <div className="w-24 h-24 bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-2xl flex items-center justify-center relative overflow-hidden">
              ...
              <div
                className="absolute inset-0 border-2 border-yellow-300/30 rounded-2xl animate-radar-ping"
            ...
            <div className="space-y-3">
              {analysis?.positiveInsights?.map((insight, idx) => (
                <InsightCard
                  ...
                  index={idx}
                  onClick={() =>
                    setSelectedIssue({ ...insight, type: "positive" })
                  }
                />
            ...
            <div className="space-y-3">
              {analysis?.issues?.map((issue, idx) => (
                <InsightCard
                  ...
                  index={idx}
                  onClick={() =>
                    setSelectedIssue({ ...issue, type: "negative" })
                  }
                />
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 relative z-10">
        {type === "positive" ? (
          <CheckCircle
            className={`w-5 h-5 ${color.icon} flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform`}
          />
        ) : (
          <AlertTriangle
            className={`w-5 h-5 ${color.icon} flex-shrink-0 mt-0.5 group-hover:scale-110 transition-transform animate-pulse`}
          />
        )}
        <div className="flex-1">
          <h4
            className={`font-semibold ${color.text} mb-1 group-hover:text-white transition-colors`}
          >
            {insight.title}
          </h4>
...
const IssueDetailModal = ({ issue, onClose }) => {
  ...
  const getSeverity = (issue) => {
    if (issue.title.includes("High Memory") || issue.title.includes("Critical"))
      return "🔴 Critical";
    if (issue.title.includes("Many") || issue.title.includes("Warning"))
      return "🟠 High";
    if (issue.title.includes("Low") || issue.title.includes("Moderate"))
      return "🟡 Medium";
    return issue.type === "positive"
      ? "✅ Informational"
  ...
  const getImpact = (issue) => {
    ...
    if (issue.title.includes("Reports"))
      return "Medium - Requires admin action";
  ...
  const getSolutions = (issue) => {
    ...
    if (issue.title.includes("High Memory") || issue.title.includes("Memory")) {
      return {
        aiAnalysis:
          "The system is experiencing elevated memory consumption. This typically occurs due to memory leaks, inefficient caching, or high concurrent user load. Immediate action is recommended to prevent service degradation.",
        rootCause: [
    ...
    if (
      issue.title.includes("Pending Reports") ||
      issue.title.includes("Many") ||
      issue.title.includes("Reports")
    ) {
      return {
        aiAnalysis:
          "A backlog of user reports has accumulated, indicating either increased user activity or insufficient moderation resources. Timely report processing is crucial for maintaining platform safety and user trust.",
        rootCause: [
    ...
    // Low Activity
    if (
      issue.title.includes("Low Activity") ||
      issue.title.includes("Inactive") ||
      issue.title.includes("Activity")
    ) {
      return {
        aiAnalysis:
          "User engagement has decreased below normal levels. This could indicate technical issues, poor user experience, or external factors affecting user behavior. Investigation is needed to identify the root cause.",
        rootCause: [
    ...
    // Suspended Users
    if (
      issue.title.includes("Suspended") ||
      issue.title.includes("Suspension")
    ) {
      return {
        aiAnalysis:
          "Expired user suspensions have not been automatically cleared, preventing legitimate users from accessing the platform. This requires immediate database cleanup to restore user access.",
        rootCause: [
    ...
    return {
      aiAnalysis:
        "An issue has been detected that requires investigation. The AI system has identified an anomaly in system behavior that deviates from normal patterns. Further analysis is recommended to determine the root cause and appropriate resolution.",
      rootCause: [
      ...
      >
        ...
        <div className="relative z-10 p-8">
          <div className="flex items-start gap-4 mb-6">
            {issue.type === "positive" ? (
              ...
              <div className="p-3 bg-red-500/20 rounded-xl">
                <AlertTriangle
                  className={`w-8 h-8 ${color.icon} animate-pulse`}
                />
              </div>
            )}
            <div className="flex-1">
          ...
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            {/* Technical Details */}
            {issue.type === "negative" &&
              getSolutions(issue).technicalDetails && (
                <div className="p-4 bg-black/30 rounded-xl border border-white/10">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <p className="text-white/90 font-medium">
                        {getSolutions(issue).technicalDetails.threshold}
                      </p>
                    </div>
                    <div>
                      <span className="text-white/50">Impact:</span>
                      <p className="text-white/90 font-medium">
                        {getSolutions(issue).technicalDetails.currentImpact}
                      </p>
                    </div>
                    <div>
                      <span className="text-white/50">Est. Time:</span>
                      <p className="text-white/90 font-medium">
                        {getSolutions(issue).technicalDetails.estimatedTime}
                      </p>
                    </div>
                    <div>
                      <span className="text-white/50">Complexity:</span>
                      <p className="text-white/90 font-medium">
                        {getSolutions(issue).technicalDetails.complexity}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-4 bg-red-500/10 rounded-xl border border-red-500/30">
                ...
                <ol className="space-y-2 text-sm text-white/90">
                  {getSolutions(issue).steps.map((step, idx) => (
                    <li
                      key={idx}
                      className="flex gap-3 p-2 bg-black/20 rounded"
                    >
            ...
            {issue.type === "positive" && (
              <div className="p-4 bg-green-500/10 rounded-xl border border-green-500/30">
                ...
                <p className="text-sm text-white/80 leading-relaxed">
                  This positive trend indicates healthy system performance.
                  Continue monitoring to maintain this level. Consider
                  documenting what's working well to replicate success in other
                  areas.
                </p>
              </div>
            )}
