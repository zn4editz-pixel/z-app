...
const RealWorldMap = ({ locationStats = [] }) => {
  ...
  const getCountryColor = (countryCode, users = 0) => {
    const hasUsers = enhancedLocationStats.find(
      (stat) => stat.code === countryCode,
    );
    if (!hasUsers) {
  ...
  const handleCountryClick = (countryCode) => {
    const countryData = enhancedLocationStats.find(
      (stat) => stat.code === countryCode,
    );
    if (countryData) {
                      ...
                      <g>
                        ...
                        <text
                          ...
                          style={{
                            fontSize: Math.max(
                              8,
                              Math.sqrt(countryData.users) / 5,
                            ),
                          }}
                        >
                          {countryData.users > 999
                            ? `${(countryData.users / 1000).toFixed(1)}k`
                            : countryData.users}
                        </text>
                      </g>
                    )}
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-semibold text-gray-900 dark:text-white">
                {enhancedLocationStats
                  .reduce((sum, country) => sum + country.users, 0)
                  .toLocaleString()}
              </span>
          ...
          {selectedCountry ? (
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 mt-3">
                    <div
                      ...
                      style={{
                        width: `${Math.min(selectedCountry.percentage * 4, 100)}%`,
                      }}
                    ></div>
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                <h6 className="font-bold text-gray-900 dark:text-white mb-3">
                  Activity Insights
                </h6>
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-amber-600">
                      {Math.floor(selectedCountry.users * 0.7).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
              ...
              <p className="text-sm">
                Click on any highlighted country to view detailed analytics
              </p>
            </div>
          )}
