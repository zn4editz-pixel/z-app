...
const AppearancePanel = () => {
  ...
  const toggleTheme = (theme) => {
    let currentThemes =
      localSettings?.allowedThemes === "all"
        ? THEMES
        : (localSettings?.allowedThemes || "").split(",");
  ...
  const isThemeAllowed = (theme) => {
    if (!localSettings?.allowedThemes || localSettings.allowedThemes === "all")
      return true;
    ...
    <div className="space-y-8 animate-fadeIn pb-20">
      ...
      <div className="bg-base-100 p-6 rounded-2xl shadow-lg border border-base-300">
        ...
        <p className="text-base-content/60">
          Customize visible themes, animations, and seasonal effects.
        </p>
      </div>
        ...
        <div className="bg-base-100 p-6 rounded-2xl shadow-lg border border-base-300">
          ...
          <div className="aspect-video bg-base-300/50 rounded-xl overflow-hidden relative mb-4 border border-base-content/10 shadow-inner">
            <div className="absolute inset-0 transform scale-[0.6] origin-center pointer-events-none">
              ...
              {localSettings?.loginAnimation === "stranger" && (
                <StrangerAnimation />
              )}
              {localSettings?.loginAnimation === "live-match" && (
                <LiveMatchAnimation />
              )}
              {localSettings?.loginAnimation === "connect" && (
                <ConnectAnimation />
              )}
            </div>
        ...
        <div className="bg-base-100 p-6 rounded-2xl shadow-lg border border-base-300">
          ...
          <div className="aspect-video bg-base-300/50 rounded-xl overflow-hidden relative mb-4 border border-base-content/10 shadow-inner">
            <div className="absolute inset-0 transform scale-[0.6] origin-center pointer-events-none">
              ...
              {localSettings?.signupAnimation === "stranger" && (
                <StrangerAnimation />
              )}
              {localSettings?.signupAnimation === "live-match" && (
                <LiveMatchAnimation />
              )}
              {localSettings?.signupAnimation === "connect" && (
                <ConnectAnimation />
              )}
            </div>
            <div className="absolute bottom-2 right-2 bg-black/50 text-white text-xs px-2 py-1 rounded backdrop-blur-sm">
      ...
      <div className="bg-base-100 p-6 rounded-2xl shadow-lg border border-base-300">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-base-content/60">
                This theme will be used for new visitors and incognito windows.
              </p>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-4">
          ...
          <div className="alert alert-info text-sm">
            ...
            <span>
              Selected theme will be the fallback for anyone who hasn't chosen a
              theme yet.
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-base-content/60">
                Select which themes are available to users.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex-1">
            ...
            <p className="text-sm text-base-content/60 mb-4">
              Enable automatic seasonal themes (e.g., Snowfall for Winter). This
              overrides the user's selected theme on the Login/Signup pages.
            </p>
            <input
              type="checkbox"
          ...
          <div className="w-full md:w-1/2 aspect-video bg-gradient-to-br from-blue-900 to-slate-900 rounded-xl relative overflow-hidden shadow-2xl border-2 border-white/20 group">
            {/* Snow Effect Preview */}
            <div
              className={`absolute inset-0 transition-opacity duration-700 ${localSettings?.isSeasonalMode ? "opacity-100" : "opacity-20 grayscale"}`}
            >
              {/* Mock Snow */}
              {[...Array(20)].map((_, i) => (
                <div
            ...
            {!localSettings?.isSeasonalMode && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                <span className="text-white/70 font-bold uppercase tracking-widest border-2 border-white/30 px-4 py-2 rounded-lg">
                  Disabled
                </span>
              </div>
            )}
