  ...
  return (
    <div className="space-y-6 relative min-h-screen">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden opacity-5">
        <div
          className="absolute top-1/4 left-1/4 w-64 h-64 bg-amber-400 rounded-full blur-3xl will-change-transform"
          style={{ animation: "float 20s ease-in-out infinite" }}
        />
        <div
          className="absolute top-1/2 right-1/4 w-96 h-96 bg-yellow-400 rounded-full blur-3xl will-change-transform"
          style={{
        ...
        />
        <div
          className="absolute bottom-1/4 left-1/3 w-80 h-80 bg-orange-400 rounded-full blur-3xl will-change-transform"
          style={{
      ...
      <div className="relative bg-gradient-to-br from-black/95 via-amber-950/30 to-black/95 backdrop-blur-2xl rounded-3xl shadow-2xl p-5 sm:p-6 lg:p-8 border-2 border-amber-500/40 overflow-hidden">
        ...
        <div
          className="absolute -bottom-20 -right-20 w-40 h-40 bg-yellow-500/30 rounded-full blur-3xl animate-pulse"
          style={{ animationDelay: "1s" }}
        />
        <div
          className="absolute top-1/2 left-1/2 w-32 h-32 bg-orange-500/20 rounded-full blur-3xl animate-pulse"
          ...
          {loadingAIReports ? (
            <div className="flex flex-col items-center justify-center py-16">
              <div className="relative">
                ...
                <div
                  className="absolute inset-0 loading loading-spinner loading-lg text-yellow-500 opacity-50"
                  style={{ animationDelay: "0.3s" }}
                />
              </div>
              <p className="text-sm text-amber-200/80 mt-4 animate-pulse">
              ...
              {aiStats && (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                  <div className="bg-transparent backdrop-blur-sm rounded-xl p-4 text-center shadow-lg border-2 border-amber-400/40 hover:border-amber-400/80 hover:shadow-xl hover:shadow-amber-500/20 hover:scale-105 transition-all duration-300 group">
                    <div className="text-3xl font-bold bg-gradient-to-r from-amber-400 to-yellow-400 bg-clip-text text-transparent group-hover:scale-110 transition-transform">
                      {aiStats.total}
                    </div>
                    <div className="text-xs font-medium text-amber-200/70 mt-1">
                      Total AI Reports
                    </div>
                  </div>
                  <div className="bg-transparent backdrop-blur-sm rounded-xl p-4 text-center shadow-lg border-2 border-yellow-400/40 hover:border-yellow-400/80 hover:shadow-xl hover:shadow-yellow-500/20 hover:scale-105 transition-all duration-300 group">
                    ...
                    <div className="text-xs font-medium text-yellow-200/70 mt-1">
                      Pending Review
                    </div>
                  </div>
                  ...
                  <div className="bg-transparent backdrop-blur-sm rounded-xl p-4 text-center shadow-lg border-2 border-green-400/40 hover:border-green-400/80 hover:shadow-xl hover:shadow-green-500/20 hover:scale-105 transition-all duration-300 group">
                    ...
                    <div className="text-xs font-medium text-green-300/70 mt-1">
                      Action Taken
                    </div>
                  </div>
                  <div className="bg-transparent backdrop-blur-sm rounded-xl p-4 text-center shadow-lg border-2 border-orange-400/40 hover:border-orange-400/80 hover:shadow-xl hover:shadow-orange-500/20 hover:scale-105 transition-all duration-300 group">
                    ...
                    <div className="text-xs font-medium text-orange-200/70 mt-1">
                      Dismissed
                    </div>
                  </div>
                  <div className="bg-transparent backdrop-blur-sm rounded-xl p-4 text-center shadow-lg border-2 border-amber-500/40 hover:border-amber-500/80 hover:shadow-xl hover:shadow-amber-600/20 hover:scale-105 transition-all duration-300 group">
                    <div className="text-2xl font-bold bg-gradient-to-r from-amber-400 to-yellow-300 bg-clip-text text-transparent group-hover:scale-110 transition-transform">
                    ...
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart
                        data={[
                          {
                            range: "90-100%",
                            count: aiReports.filter(
                              (r) => r.aiConfidence >= 0.9,
                            ).length,
                          },
                          {
                            range: "80-89%",
                            count: aiReports.filter(
                              (r) =>
                                r.aiConfidence >= 0.8 && r.aiConfidence < 0.9,
                            ).length,
                          },
                          {
                            range: "70-79%",
                            count: aiReports.filter(
                              (r) =>
                                r.aiConfidence >= 0.7 && r.aiConfidence < 0.8,
                            ).length,
                          },
                          {
                            range: "60-69%",
                            count: aiReports.filter(
                              (r) =>
                                r.aiConfidence >= 0.6 && r.aiConfidence < 0.7,
                            ).length,
                          },
                          {
                            range: "<60%",
                            count: aiReports.filter((r) => r.aiConfidence < 0.6)
                              .length,
                          },
                        ]}
                      >
                        ...
                        <XAxis
                          dataKey="range"
                          style={{ fontSize: "12px", fill: "#fde68a" }}
                        />
            ...
            {aiReports.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-xl font-bold text-green-400">
                  No AI-detected violations!
                </p>
                <p className="text-sm text-amber-200/60 mt-2">
                  The system is monitoring content in real-time
                </p>
              </div>
            ) : (
              <>
                  ...
                  <table className="table w-full table-zebra">
                    ...
                    <tbody>
                      {aiReports.map((report) => (
                        <tr
                          key={report.id}
                          className="text-center hover:bg-base-200/50 transition-all"
                        >
                          ...
                          <td>
                            <span
                              className={`badge badge-md font-semibold shadow-md ${
                ...
                <div className="lg:hidden space-y-4">
                  {aiReports.map((report) => (
                    <div
                      key={report.id}
                      className="bg-gradient-to-br from-base-200/50 to-base-300/30 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-base-300 hover:shadow-xl transition-all"
                    >
                      {/* Header */}
                          ...
                          <div className="flex flex-col items-center gap-2">
                            <div className="avatar">
                              <div className="w-16 h-16 rounded-full ring-2 ring-success ring-offset-2 ring-offset-base-100">
                                <img
                                  src={
                                    report.reporter?.profilePic || "/avatar.png"
                                  }
                                  alt="Reporter"
                                />
                              </div>
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {report.status === "reviewed" && (
                          <>
                            <button
                              onClick={() =>
                                onUpdateReportStatus(report.id, "action_taken")
                              }
