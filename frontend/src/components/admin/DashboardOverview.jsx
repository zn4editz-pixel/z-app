    ...
    return (
      <div className="space-y-6 relative min-h-screen">
        <div className="flex flex-col items-center justify-center py-16 relative z-10">
          ...
          <p className="text-sm text-warning/80 mt-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 relative z-10">
        ...
        <div className="bg-gradient-to-br from-base-100/90 to-base-200/90 backdrop-blur-sm rounded-2xl p-6 shadow-xl hover:shadow-2xl transition-all duration-200 hover:scale-[1.02] border border-warning/20">
          <div className="flex items-center justify-between mb-4">
            <div className="text-base-content/70 text-sm font-bold uppercase tracking-wide">
              Total Users
            </div>
          ...
          <div className="text-sm text-base-content/60 font-medium">
            +{safeStats?.recentUsers || Math.floor(safeUsers.length * 0.3)} this
            week
          </div>
        </div>
        {/* Online Now Card */}
        <div className="bg-gradient-to-br from-base-100/90 to-base-200/90 backdrop-blur-sm rounded-2xl p-6 shadow-xl hover:shadow-2xl transition-all duration-200 hover:scale-[1.02] border border-success/20">
          ...
          <div className="text-4xl font-bold mb-1 text-success">
            {safeStats?.onlineUsers ||
              safeUsers.filter((u) => u.isOnline).length}
          </div>
          <div className="text-sm text-base-content/60 font-medium">
            {safeStats?.verifiedUsers ||
          ...
          <div className="max-h-64 overflow-y-auto space-y-2">
            ...
            {safeUsers.filter((u) => u.isOnline).length > 10 && (
              <div className="text-center py-2 text-base-content/60 text-sm">
                +{safeUsers.filter((u) => u.isOnline).length - 10} more users
                online
              </div>
            )}
          </div>
              ...
              safeUsers
                ...
                .map((user) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-xs font-medium ${user.isOnline ? "text-success" : "text-base-content/60"}`}
                    >
    ...
    intervalRef.current = setInterval(() => {
      ...
      setAnalyticsData({
        userActivity: Math.min(
          100,
          (onlineUsers / Math.max(totalUsers * 0.3, 1)) * 100,
        ),
        engagement: Math.min(
          100,
          60 + Math.sin(Date.now() / 2000) * 20 + Math.random() * 15,
        ),
        performance: Math.min(
          100,
          75 + Math.cos(Date.now() / 1500) * 15 + Math.random() * 10,
        ),
        growth: Math.min(
          100,
          45 + Math.sin(Date.now() / 3000) * 25 + Math.random() * 20,
        ),
      });
      ...
      setWaveData((prev) =>
        prev.map((point, i) => ({
          ...
          y:
            50 +
            Math.sin(Date.now() / 1000 + i * 0.2) * 15 +
            Math.sin(Date.now() / 500 + i * 0.1) * 8,
        })),
      );
    }, 200);
    return () => {
  ...
  const CircularProgress = ({
    ...
    return (
      <div className="relative flex flex-col items-center">
        ...
        <span className="text-xs text-base-content/70 mt-2 font-medium">
          {label}
        </span>
      </div>
    );
    ...
    intervalRef.current = setInterval(() => {
      setMetricsData((prev) => {
        ...
        const newPoint = {
          ...
          cpu: Math.max(
            10,
            40 + Math.sin(baseTime / 10000) * 20 + Math.random() * 15,
          ),
          memory: Math.max(
            30,
            60 + Math.sin(baseTime / 12000) * 15 + Math.random() * 10,
          ),
          network: Math.max(
            5,
            30 + Math.sin(baseTime / 8000) * 25 + Math.random() * 20,
          ),
