...
const ChatMessage = ({
  ...
  const handleTouchMove = (e) => {
    ...
    if (Math.abs(deltaX) > 10 && deltaY < 30) {
      ...
      if (
        Math.abs(newOffset) >= SWIPE_THRESHOLD &&
    ...
    if (typeof window !== "undefined" && messageRef.current) {
      ...
      if (isMyMessage) {
        window[`message_${message.id}`] = {
          ...
          testDelivered: () =>
            window.testMessageStatus(message.id, "delivered"),
        };
      }
    }
  }, [
  ...
  const isEmojiOnly =
    message.text &&
    /^[\p{Emoji}\s]+$/u.test(message.text) &&
    !message.image &&
    !message.voice;
  const emojiCount = message.text
    ? (message.text.match(/\p{Emoji}/gu) || []).length
    : 0;
  ...
  if (message.isDeleted) {
    return (
      <div
        className={`flex flex-col ${isMyMessage ? "items-end" : "items-start"} mb-3 px-3`}
      >
              ...
              >
                ...
                {message.replyTo && (
                  ...
                  >
                    ...
                    <div
                      className={`absolute left-0 top-0 bottom-0 w-1 rounded-full ${isMyMessage ? "bg-primary-content/80" : "bg-primary/80"}`}
                    />
                    <div className="pl-3">
                        ...
                        {message.replyTo.isCallLog ? (
                          <div className="flex items-center gap-1.5">
                            <span className="text-sm">
                              {message.replyTo.callType === "video"
                                ? "📹"
                                : "📞"}
                            </span>
                            <span>
                      ...
                      {message.replyTo.replyTo && (
                        ...
                        >
                          ...
                          <span>
                            Reply to{" "}
                            {message.replyTo.replyTo.senderId === authUser.id
                              ? "You"
                    ...
                    {message.voice && (
                      <div className="flex items-center gap-2 min-w-[200px] max-w-[280px]">
                        ...
                        <div className="flex-1 flex items-center justify-center gap-[3px] h-8">
                          {[
                            4, 7, 5, 9, 6, 10, 7, 8, 5, 9, 6, 7, 8, 6, 9, 7, 5,
                            8, 6, 7,
                          ].map((height, i) => {
            ...
            {Object.keys(groupedReactions).length > 0 && (
              ...
              >
                {Object.entries(groupedReactions).map(([emoji, users]) => {
                  ...
                  const badgeStyle = getReactionBadgeStyle(
                    hasMyReaction,
                    themeColors,
                  );
                  return (
