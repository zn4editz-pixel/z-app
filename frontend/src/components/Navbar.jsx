...
import { LayoutDashboard, Settings, User, Compass } from "lucide-react";
const Navbar = () => {
  ...
  return (
    <header className="bg-base-100/80 backdrop-blur-lg border-b border-base-300 fixed w-full top-0 z-40 shadow-sm transition-all duration-300">
      <div className="px-3 sm:px-4 lg:px-6 h-14 sm:h-16">
        <div className="flex items-center justify-between h-full mx-auto">
          ...
          <Link
            to="/"
            className="flex items-center active:opacity-70 transition"
            aria-label="Go to home page"
          >
            <div className="navbar-logo-adaptive" aria-label="Z-APP" />
          </Link>
          {/* Navigation Icons */}
          <div className="flex items-center -space-x-2 sm:space-x-0 sm:gap-1">
            ...
            {authUser && (
              ...
              >
                ...
                {totalUpdates > 0 && (
                  <span
                    className="absolute -top-0.5 -right-0.5 bg-error text-white text-[10px] sm:text-xs font-bold rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center shadow-lg animate-pulse"
                    aria-label={`${totalUpdates} new updates`}
                  >
