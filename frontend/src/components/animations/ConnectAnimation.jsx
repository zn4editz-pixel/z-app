...
import { Lock, Unlock, MessageSquare, Video, UserPlus, Send, Hand, ThumbsUp, ShieldCheck } from "lucide-react";
...
const ConnectAnimation = () => {
    ...
    const runCycle = async () => {
        ...
        setMessages(prev => [...prev, { text: "Hey! Nice to meet you! 👋", sender: "other" }]);

        await new Promise(r => setTimeout(r, 1200));
        setMessages(prev => [...prev, { text: "Same here! Love the vibe.", sender: "me" }]);

        await new Promise(r => setTimeout(r, 1200));
        setMessages(prev => [...prev, { text: "Accepted the friend request! 🤝", sender: "other" }]);
    ...
    return (
        <div className="relative w-full h-full bg-[#121212] overflow-hidden flex flex-col items-center justify-center font-sans">
            ...
            {/* BACKGROUND ALIVE */}
            <div className={`absolute inset-0 transition-colors duration-1000 ${stage === 2 ? 'bg-base-200' : 'bg-[#121212]'}`}>
            ...
            <div className={`absolute inset-0 flex items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.33,1,0.68,1)] transform
            ...
            `}>
                ...
                <div className="absolute -translate-y-[110px] w-48 h-32 bg-black rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
                    <img src={charA.img} className="w-full h-full object-cover opacity-80" />
                ...
                <div className="absolute translate-y-[110px] w-48 h-32 bg-black rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
                    <img src={charB.img} className="w-full h-full object-cover opacity-80" />
                    <div className="absolute bottom-2 left-2 bg-black/50 px-2 py-1 rounded text-[10px] text-white backdrop-blur-sm">
                        {charB.name}
                    </div>
                ...
                <div className="absolute z-20 flex flex-col items-center">
                    {/* Locked State */}
                    <div className={`transition-all duration-300 ${stage === 0 ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
                    ...
                    <div className={`absolute transition-all duration-300 ${stage === 1 ? 'scale-125 opacity-100' : 'scale-0 opacity-0'}`}>
            ...
            <div className={`absolute w-full max-w-xs transition-all duration-700 ease-[cubic-bezier(0.33,1,0.68,1)]
                ...
                <div className="bg-base-100 rounded-3xl shadow-2xl border border-base-content/5 overflow-hidden flex flex-col h-[400px]">
                    ...
                    <div className="h-16 bg-base-100 border-b border-base-200 flex items-center px-4 justify-between">
                        <div className="flex items-center gap-3">
                            <div className="relative">
                                <img src={charB.img} className="w-10 h-10 rounded-full object-cover border-2 border-primary" />
                    ...
                    <div className="flex-1 p-4 space-y-4 overflow-hidden bg-base-200/50 relative">
                        ...
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
                                <div className={`
                    ...
                    <div className="p-3 bg-base-100 border-t border-base-200">
                        <div className="h-10 bg-base-200 rounded-full flex items-center px-4 gap-2 border border-transparent focus-within:border-primary/50 transition-colors">
                            <input className="flex-1 bg-transparent text-xs outline-none" placeholder="Message..." disabled />
