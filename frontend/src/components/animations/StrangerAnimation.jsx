    ...
    useEffect(() => {
        ...
        const runCycle = async () => {
            while (isMounted) {
                ...
                // Message 2
                await new Promise(r => setTimeout(r, 1500));
                setChatMessages(prev => [...prev, { text: "Thanks! Nice to meet you too! 😄", sender: "me" }]);
      ...
      */}
            ...
            `}>
                ...
                `}>
                    ...
                    {/* CARD A (Top) */}
                    <div className={`absolute left-1/2 -translate-x-1/2 w-40 h-56 bg-white/10 backdrop-blur-md rounded-2xl p-1 border border-white/20 transition-all duration-1000 ease-[cubic-bezier(0.22,1,0.36,1)] shadow-2xl z-20
                ...
                <div className={`absolute top-8 z-30 transition-all duration-500 flex flex-col items-center gap-2 ${stage === 2 ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
                ...
                <div className={`absolute top-8 z-30 transition-all duration-500 flex flex-col items-center gap-2 ${stage === 3 ? 'opacity-100 translate-y-0 scale-110' : 'opacity-0 -translate-y-4 scale-90'}`}>
                ...
                <div className={`absolute bottom-4 left-0 right-0 z-40 px-6 space-y-3 transition-opacity duration-500 ${stage === 4 ? 'opacity-100' : 'opacity-0'}`}>
                ...
                {(stage === 2 || stage === 3) && (
                    <>
                        <div className="absolute text-4xl animate-bounce delay-700 right-10 top-1/3 z-30">👋</div>
                        <div className="absolute text-4xl animate-pulse delay-1000 left-10 bottom-1/3 z-30">😄</div>
                    </>
                )}
