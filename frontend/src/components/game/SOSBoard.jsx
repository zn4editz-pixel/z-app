...
const SOSBoard = () => {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      const remaining = Math.max(
        0,
        Math.ceil((gameState.turnExpiresAt - now) / 1000),
      );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div
        ref={containerRef}
        className="bg-base-100 w-[90%] max-w-[320px] sm:max-w-[360px] md:max-w-[400px] rounded-2xl shadow-2xl border border-primary/20 overflow-hidden flex flex-col relative my-auto"
      >
        {/* Header */}
        <div className="p-3 bg-base-200/50 flex justify-between items-center border-b border-base-300">
          <div className="flex items-center gap-2">
            <div
              className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full ${isMyTurn ? "bg-green-500 animate-pulse" : "bg-gray-400"}`}
            />
        ...
        <div className="flex justify-between px-4 py-2 sm:py-4 bg-gradient-to-b from-base-200/30 to-transparent">
          <div
            className={`flex flex-col items-center ${gameState.currentTurn === authUser.id ? "scale-105 text-primary" : "opacity-60"} transition-all duration-300`}
          >
          ...
          <div className="flex flex-col justify-center items-center px-2">
            ...
            <div
              className={`badge badge-xs sm:badge-sm mb-2 ${gameState.status === "playing" ? "badge-info" : "badge-ghost"}`}
            >
              {gameState.status === "playing"
                ? isMyTurn
                  ? "YOUR TURN"
            ...
            {gameState.status === "playing" && (
              <div
                className={`radial-progress text-[10px] font-bold transition-all duration-300 ${timeLeft <= 5 ? "text-error scale-110" : "text-primary"}`}
          ...
          <div
            className={`flex flex-col items-center ${gameState.currentTurn === opponentId ? "scale-105 text-secondary" : "opacity-60"} transition-all duration-300`}
          >
            <div className="avatar mb-1 pointer-events-none">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-current shadow-md">
                <img
        ...
        <div className="p-3 sm:p-6 flex-1 flex flex-col items-center justify-center bg-base-100 relative">
          ...
          {gameState.status !== "playing" && (
            <div className="absolute inset-0 z-40 bg-black/40 backdrop-blur-[2px] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
              <div
                className={`card w-full max-w-xs shadow-2xl overflow-hidden border-2 ${
                  gameState.winner === authUser.id
                    ? "bg-gradient-to-br from-yellow-100 to-amber-200 border-yellow-400"
                ...
                <div className="card-body items-center text-center p-4">
                  {gameState.winner === authUser.id ? (
                    <>
                      ...
                      <h2 className="card-title text-xl font-black text-amber-600">
                        VICTORY!
                      </h2>
                    ...
                    <>
                      ...
                      <h2 className="card-title text-xl font-black text-base-content">
                        DRAW
                      </h2>
                    </>
                  ) : (
                    <>
                      ...
                      <h2 className="card-title text-xl font-black text-base-content/60">
                        DEFEAT
                      </h2>
                    </>
                  )}
                  ...
                  <div className="card-actions mt-3 w-full">
                    <button
                      onClick={handleClose}
                      className={`btn btn-sm w-full rounded-xl border-none shadow-md ${
                        gameState.winner === authUser.id
            ...
            {gameState.board.map((row, rIndex) =>
              row.map((cell, cIndex) => (
                ...
                >
                  {cell && (
                    <span
                      className={`animate-in zoom-in spin-in-12 duration-300 ${cell === "S" ? "text-primary drop-shadow-md" : "text-secondary drop-shadow-md"}`}
                    >
                      {cell}
                    </span>
                  )}
            ...
            {gameState.lines && gameState.lines.length > 0 && (
              ...
              >
                {gameState.lines.map((line, i) => {
                  ...
                  const color =
                    line.playerId === authUser.id ? "#10B981" : "#EF4444"; // Green for me, Red for opponent
                  return (
                    <line
                      key={`line-${i}`}
                      x1={x1}
...
if (
  ...
  !document.getElementById("sos-line-animation-fixed")
) {
