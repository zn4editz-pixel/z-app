...
const MessageDiagnosticPage = () => {
  ...
  const addResult = (test, status, message) => {
    setTestResults((prev) => [
      ...prev,
      { test, status, message, time: new Date().toLocaleTimeString() },
    ]);
  };
  const runDiagnostics = async () => {
    ...
    addResult(
      "Starting",
      "info",
      "Running comprehensive message sending diagnostics...",
    );
    ...
    addResult(
      ...
      authUser
        ? `Logged in as: ${authUser.username || authUser.fullName}`
        : "Not authenticated",
    );
    ...
    try {
      ...
      addResult(
        "Users API",
        "success",
        `Found ${usersResponse.data.length} users`,
      );
      if (usersResponse.data.length > 0) {
        ...
        addResult(
          "Target User",
          "success",
          `Target: ${testUser.username || testUser.nickname} (${testUser.id})`,
        );
        // Test message sending via API
        const messageResponse = await axiosInstance.post(
          `/messages/send/${testUser.id}`,
          {
        ...
        addResult(
          "API Message Send",
          "success",
          `Message sent via API: ${messageResponse.data.id}`,
        );
      }
    } catch (error) {
    ...
    if (socket?.connected && selectedUser) {
      ...
      addResult(
        "Socket Test",
        "error",
        "Cannot test socket - not connected or no selected user",
      );
    }
        ...
        <div className="bg-base-100 rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="bg-base-200 p-4 rounded-lg">
              ...
              <div
                className={`badge ${diagnostics.authUser ? "badge-success" : "badge-error"}`}
              >
                {diagnostics.authUser ? "Authenticated" : "Not Authenticated"}
              </div>
              {authUser && (
                <p className="text-sm mt-1">
                  User: {authUser.username || authUser.fullName}
                </p>
              )}
            </div>
            <div className="bg-base-200 p-4 rounded-lg">
              ...
              <div
                className={`badge ${diagnostics.socketConnected ? "badge-success" : "badge-error"}`}
              >
            ...
            <div className="bg-base-200 p-4 rounded-lg">
              ...
              <div
                className={`badge ${diagnostics.selectedUser ? "badge-success" : "badge-warning"}`}
              >
                {diagnostics.selectedUser
                  ? "User Selected"
                  : "No User Selected"}
              </div>
              {selectedUser && (
                <p className="text-sm mt-1">
                  User: {selectedUser.username || selectedUser.fullName}
                </p>
              )}
            </div>
            <div className="bg-base-200 p-4 rounded-lg">
              ...
              <div
                className={`badge ${diagnostics.token ? "badge-success" : "badge-error"}`}
              >
          ...
          <div className="mb-6">
            <button onClick={runDiagnostics} className="btn btn-primary">
