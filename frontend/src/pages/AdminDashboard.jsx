...
const AdminDashboard = () => {
  ...
  const [loadingAIReports, setLoadingAIReports] = useState(false);
  ...
  const fetchAIReports = async () => {
    ...
    try {
      ...
      if (err.response?.status === 403) {
        ...
        toast.error(
          err.response?.data?.error || "Failed to load AI moderation data",
        );
      }
      setAIReports([]);
      setAIStats(null);
  ...
  const handleDeleteUser = async (userId) => {
    if (
      !confirm(
        "Are you sure you want to permanently delete this user? This will delete all their messages, friend requests, and related data. This action cannot be undone.",
      )
    )
      return;
    try {
  ...
  const handleToggleVerification = async (userId) => {
    try {
      ...
      toast.success(
        response.data?.message || "Verification status updated successfully",
      );
  ...
  const handleUpdateReportStatus = async (reportId, newStatus) => {
    try {
      ...
      // Refresh all data
      await Promise.all([fetchReports(), fetchAIReports(), fetchStats()]);
    } catch (err) {
      toast.error(
        err.response?.data?.error ||
          err.response?.data?.message ||
  ...
  const handleRejectVerification = async (userId) => {
    ...
    setVerificationRequests((prev) => prev.filter((req) => req.id !== userId));
    try {
      await axiosInstance.put(`/admin/verification/reject/${userId}`, {
        reason,
      });
      toast.success("Verification rejected");
      // Refetch to ensure consistency
      await Promise.all([fetchVerificationRequests(), fetchStats()]);
    } catch (err) {
      toast.error(
  ...
  const handleSendNotification = async ({ type, title, message, userId }) => {
    try {
      if (type === "broadcast") {
        ...
        await axiosInstance.post(`/admin/notifications/personal/${userId}`, {
          title,
          message,
        });
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      ...
      case "notifications":
        return (
          <NotificationsPanel onSendNotification={handleSendNotification} />
        );
      case "system-health":
    ...
    <div className="min-h-screen pt-14 xs:pt-16 sm:pt-18 md:pt-20 px-2 xs:px-3 sm:px-4 lg:px-6 bg-gradient-to-br from-base-200 via-base-300 to-base-200 pb-16 xs:pb-18 sm:pb-20 md:pb-10 relative admin-performance-mode">
      ...
      <div className="admin-enhanced-background">
        ...
        <div className="status-nodes">
          <div
            className="status-node node-backend status-healthy"
            title="Backend - Healthy"
          ></div>
          <div
            className="status-node node-frontend status-info"
            title="Frontend - Info"
          ></div>
          <div
            className="status-node node-database status-warning"
            title="Database - Warning"
          ></div>
          <div
            className="status-node node-websocket status-critical"
            title="WebSocket - Critical"
          ></div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all duration-200 whitespace-nowrap text-xs font-medium ${
                    activeTab === tab.id
        ...
        {/* Tab Content */}
        <div className="animate-fadeIn relative">{renderActiveTab()}</div>
      </div>
    </div>
  );
};
