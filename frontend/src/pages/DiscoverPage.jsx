...
const AdminNotificationsList = () => {
    ...
    const loadNotifications = async () => {
      ...
      try {
        ...
        sessionStorage.setItem(
          "adminNotifications",
          JSON.stringify(notificationsToAdd),
        );
  ...
  useEffect(() => {
    const adminNotes = notifications.filter(
      ...
      {adminNotifications.map((notification) => (
        ...
        >
          <div className="flex items-start gap-3">
            <div
              className={`flex-shrink-0 ${getTextColor(notification.color)}`}
            >
              {getIcon(notification.notificationType)}
            </div>
            <div className="flex-1 min-w-0">
              <h4
                className={`font-bold text-sm sm:text-base ${getTextColor(notification.color)} mb-1`}
              >
                {notification.title}
              </h4>
    ...
    async (userId, event) => {
      ...
      try {
        ...
        setButtonStates(
          (prev) =>
            new Map(
              prev.set(userId, { isProcessing: true, timestamp: Date.now() }),
            ),
        );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-base-100 rounded-lg sm:rounded-xl shadow-lg p-4 sm:p-6 mb-4 sm:mb-6">
          <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-4">
            ...
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold bg-gradient-to-r from-base-content via-warning to-base-content bg-clip-text text-transparent animate-gradient bg-[length:200%_auto]">
              Social Hub
            </h1>
          </div>
        ...
        <div className="bg-base-100 rounded-lg sm:rounded-xl shadow-lg mb-4 sm:mb-6 overflow-hidden">
          <div className="flex border-b border-base-300 relative">
            ...
            >
              <Search
                className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ${activeTab === "discover" ? "scale-110" : ""}`}
              />
              <span>Discover</span>
            </button>
            <button
            ...
            >
              <UserCheck
                className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ${activeTab === "requests" ? "scale-110" : ""}`}
              />
              <span>Requests</span>
            ...
            >
              <Bell
                className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ${activeTab === "notifications" ? "scale-110" : ""}`}
              />
              ...
              {isLoading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {[...Array(4)].map((_, i) => (
                    <div
                      key={i}
                      className="card bg-base-200 border border-base-300 animate-pulse"
                    >
            ...
            <div className="bg-base-100 rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                {authUser?.verificationRequest?.status &&
                  authUser.verificationRequest.status !== "none" && (
                    <div
                      className={`alert text-sm sm:text-base ${authUser.verificationRequest.status === "pending"
                        ? "alert-warning"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {authUser.verificationRequest.status === "pending" && (
                          <p className="text-xs sm:text-sm mt-2 opacity-90">
                            ⏳ Your verification request is being reviewed by
                            our admin team.
                          </p>
                        )}
                        {authUser.verificationRequest.status === "approved" && (
                          ...
                          <div className="mt-2">
                            ...
                            {authUser.verificationRequest.adminNote ? (
                              ...
                              <p className="text-xs sm:text-sm opacity-90">
                                Your request does not meet verification
                                criteria.
                              </p>
                            )}
                            <p className="text-xs mt-2 opacity-75">
                              You can submit a new request after addressing the
                ...
                {authUser?.isSuspended && (
                  <div className="alert alert-error text-sm sm:text-base">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs mt-2 opacity-75">
                        You will be able to access your account again after the
